...
| force-budget | 9 | ATE BE CSE DE DPP DrE ME PE (8) | ForceReserveService (shared svc; one cached computation) + MaintenanceFacts basis |
...
| solo-formation | 14 | ATE BE CDSE CSE DE FS (6) | FormationSafety (shared common/, both bots) — LAWS only; veto enforcement plumbing = SVC-SAFETY, see loop-safety FS-enforcement row (gate 5e290559c #3) |
...
| V27-battle-reserve | V27 | BATTLE-1 | BE:858 (#evaluate, block 801-866) | BANDED (SCORE) | -40 pile<2 / -15 pile<4 (no-DTF branch) | Battle-interrupt Force readiness cost on the initiation score. Marker `"V27 BATTLE FORCE WARNING"` ×1/bot. SIBLING V27 ARMS not this row (pending own rows, §5 sweep note): PE maintenance reserve (`"V27 MAINTENANCE RESERVE"` ×1/bot) + ME maintenance move block (`"V27 MAINTENANCE MOVE BLOCK"` ×1/bot) = force-budget; ME buddy-protect (`"V27 BUDDY PROTECT"` ×2/bot) = solo-formation candidate, NOT silently reassigned. Fixture: TODO `B0_V27_BattleForce_LowPile` | LIVE (split 2026-07-13) |
...
| Tag/arm | Arm of | Sect | Anchor | KIND | Magnitude / verdict | Trigger | Status |
|---|---|---|---|---|---|---|---|
| V37.4-pass | V37.4 | pass-cancel | PE:170 (#evaluate) | BANDED (SCORE) | -50 - (hand-10)*20 on Pass, extra -100 when forcePile >= 8 (DEPLOY phase, hand >= 10 only) | HAND BLOAT: passing IS the decision being scored — pass-cancel owns it, DEPLOY-1 does not (gate ruling). Marker `"V37.4 HAND BLOAT: %d cards in hand"` ×1/bot. Fixture: TODO `B0_V374_HandBloat_PassPenalty` | LIVE (split 2026-07-13) |
...
40 base tags have live arms in 2+ domains (V24.2 added 2026-07-13 by the exact-arm split). Every arm is a separate registry row above; migrate ARMS, not tags.
...
## 5. Exact-arm registry — the 21 ex-AMBIG arms (gate 5e290559c applied 2026-07-13)
...
- Post-split live total: **364 arms** (was 357; +7: V172 +2, V169 +1, V27.1 +1, V67z +1, V24.2 +1, V193 +1) plus 1 INERT arm tracked in place (V37.4-empty-check, pass-cancel table — excluded from live counts).
- **V27 scope note (carried, still open)**: only the BE battle-reserve arm is among the 21. The PE maintenance reserve (`"V27 MAINTENANCE RESERVE"` ×1/bot) + ME maintenance move block (`"V27 MAINTENANCE MOVE BLOCK"` ×1/bot) (force-budget) and the ME buddy-protect arm (`"V27 BUDDY PROTECT"` ×2/bot; solo-formation candidate) still need their OWN rows — queued in the marker sweep, not silently reassigned.
...
- **364 live rules** (arms) across **22 domains** after the 2026-07-13 exact-arm split (was 357; +7 from splitting the 13 ex-AMBIG rows into 21 arms, §5); plus 1 INERT arm tracked in place (V37.4-empty-check, pass-cancel table — excluded from live counts, retirement pending its no-consumer fixture); 13 dead/deleted/inert/unresolved rows tracked in §6.
- Rules per domain: move 49, deploy-siting 38, deck-playbook 37, pull-search 33, deploy-attach 25, deploy-sequencing 23, battle-weapons 21, setup-starting 15, solo-formation 14, battle-forfeit 13, shields 13, battle-initiation 12, drain-control 11, loop-safety 10, force-budget 9, force-loss-payment 8, activation-amount 7, objective-intent 7, draw-count 6, response-routing 6, fact-services 5, pass-cancel 2.
- Rules per file (live; a rule counts once per owning file): CSE 138, ATE 127, DE 90, ME 35, DO 21, RCA 17, BE 14, CDSE 13, OA 10, DC 10, DrE 10, FAE 8, AA 8, SS 7, CE 6, FS 4, PE 3, DPS 3, DPP 3, BP 2, ODT 2, ObjectiveType.java 1, RandoConfig.java 1, DSf 1, EvaluatedAction.java 1. (Delta-adjusted 2026-07-13 from the batch-0 baseline for the 13→21 split: CSE +2 V172 arms; CE -2 comment-only V27/V27.1 anchor hits dropped; ME -1 and PE -1 V27 maintenance siblings now prose-tracked pending own rows; DE -1 V37.4 inert arm excluded from live counts; EvaluatedAction.java +1 FS-enforcement.)
- **40 multi-domain base tags** (§4) — the true migration workload: each needs arm-by-arm surgery.
