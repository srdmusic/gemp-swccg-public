...
| Tag/arm | Flag |
|---|---|
| V37.4 | RESOLVED: pass-cancel — marker `"V37.4 HAND BLOAT: %d cards in hand"` @ PassEvaluator.java#evaluate — the only LIVE arm scores THE PASS ACTION (deploy-phase hand-bloat penalty on Pass), squarely pass-cancel. The DE arm is INERT at current tree: `canDeployToOpponents` (DE ~3270) is computed but never read, and the empty-site penalty it modulated is hardcoded 0.0f (V40 neutralization) — no live deploy-sequencing behavior remains |
| V156 | RESOLVED: solo-formation — marker `"V156 SOLO HOLD"` @ CharacterDeploySiteEvaluator.java#computeTeamViability (shared CDSE, called from evaluateSite; no mirror) — -600 hold / +250 prefer-buddy IS solo/buddy discipline per the §1 domain definition; the arm stays physically in the CDSE hub but the RULE is owned by solo-formation. JOIN-GROUP twins (ME + CSE dest, shared isV156FlipNotReady) travel with it |
| V169-deploy | RESOLVED: solo-formation — CSE arm marker `"V169 PROTECT (deploy)"` @ CardSelectionEvaluator.java#evaluateDeployLocation; DE umbrella arm marker `"V169 PROTECT URGENT: our characters at"` @ DeployEvaluator.java#evaluate — protect-endangered-solo intent (deploy buddies to outpowered friendlies) is buddy discipline, not generic siting |
| V172 | RESOLVED: solo-formation — marker `"V172 (Steve, 2026-06): WINNABILITY GATE"` @ CardSelectionEvaluator.java#evaluateDeployLocation (`"V172 SOLO DOMINANCE"` rejected as marker: 3 hits per bot file) — it IS the dominance-exemption law: FS-L3/L4 cite the 2x V172 ruling as their exemption; it gates V171/V169-deploy but awards no siting points of its own |
| V174 | RESOLVED: force-budget — marker `"tableMaint + interruptReserve + thisMaintCost"` @ CardSelectionEvaluator.java#v173WaveProjection — pure reservation math (table upkeep + battle-interrupt force off the top of the wave budget, V177-capped); lives inside the V173 fact-helper, so migration moves OWNERSHIP, not the helper |
| V29.5-buddy | RESOLVED: solo-formation — marker `"V29.5: GENERAL BUDDY SYSTEM"` @ CardSelectionEvaluator.java#evaluateDeployLocation — the +40/-150/-100/+10 solo-to-opponent-location tiers are buddy discipline. NOTE: registry line refs (CSE 2828-2939) drifted; live block is CSE ~3089-3200 at current tree |
| V27 | RESOLVED: force-budget — homed BE arm marker `"V27 BATTLE FORCE WARNING"` @ BattleEvaluator.java#evaluate; reservation twins `"V27 MAINTENANCE RESERVE"` @ PassEvaluator.java#evaluate and `"V27 MAINTENANCE MOVE BLOCK"` @ MoveEvaluator.java#evaluate — the arm scores interrupt-fee/upkeep RESERVATIONS (-40/-15 warnings), not initiation go/no-go. Scope: the separate ME buddy-protect arm (`"V27 BUDDY PROTECT"`, ME ~822-933) is NOT this row — flagged for its own home (solo-formation candidate), not silently moved |
| V27.1 | RESOLVED: force-budget — BE arm marker `"V27.1 DTF ACTIVE"` @ BattleEvaluator.java#evaluate; PE arm marker `"V27.1 DTF RESERVE"` @ PassEvaluator.java#evaluate — Draw Their Fire tax reservation on both routes; pure "reserve N Force for X" mechanics |
| V24.2 | RESOLVED per arm (MISFIT confirmed — neither arm is battle): CSE arm → pull-search, marker `"V24.2 PULL: Lando gets +250"` @ CardSelectionEvaluator.java#evaluateTakeIntoHand (Lando/Lobot reserve-pull priority with the interleaved V47 not-alone guard); ATE arm → drain-control, marker `"V24.2 FORCE DRAIN BONUS"` @ ActionTextEvaluator.java#evaluate (+1 drain acceptance). The V24.2E comment tag (CSE ~3456) belongs to V24.14B-spy-detect (fact-services), not this row |
| V193-steer | RESOLVED: deploy-siting — DE arm marker `"V193 FLIP-GATE CONTROL: steer one body to"` @ DeployEvaluator.java#evaluate; CS arm marker `"V193 (CS) FLIP-GATE CONTROL: steer one ability body"` @ CardSelectionEvaluator.java#evaluateDeployLocation — the +400 destination bonus is siting; objective-XOR preserved: the INTEL (which site is the flip gate) stays V193-intel in ObjectiveAnalyzer (objective-intent), the steer arms only consume it |
| FS-enforcement | RESOLVED: solo-formation — primary marker `"FORMATION SAFETY: ALL actions vetoed and pass is legal"` @ CombinedEvaluator.java#evaluateDecision; point markers `"FORMATION SAFETY 2026-07-11c"` (bucket-walk veto filter) and `"epilogue must not resurrect vetoed actions"` (V67bc epilogue skip), each 1 hit/bot file — §1's own solo-formation definition names "FormationSafety (shared common/, both bots) + CE enforcement" as the target owner, and loop-safety's definition (loop breaking, retry budgets, concede, dormant checks) covers none of these points. The all-veto fallback REUSES V148 cancellability semantics; V148 itself stays SVC-SAFETY — dependency, not co-ownership |
| V141 | RESOLVED: force-budget — marker `"V141 TRANSPORT INTERRUPT BLOCK"` @ ActionTextEvaluator.java#evaluate — a 4-Force affordability floor (-2000 hold) on destiny-cost transport interrupts; reservation/floor mechanics, not movement scoring (it blocks PLAYING the interrupt, never scores destinations) |
| V67z | RESOLVED: force-budget — DrE arm marker `"V67z TRANSIT RESERVE: {} Jedi at Underground Corridor"` @ DrawEvaluator.java#calculateForceToReserve; DE twin markers `"V67z DEPLOY TRANSIT RESERVE: {} Jedi at Underground Corridor"` (budget calc) and `"V67z TRANSIT RESERVE: Deploy costs %d"` (per-deploy -1500), both @ DeployEvaluator.java#evaluate — +1/Jedi reservation mechanics; Hidden Path is the TRIGGER, not the mechanic (same pattern as V174/V27 owning reservations regardless of deck) |

### AMBIG RESOLUTION 2026-07-13 (Codex m00235 point 1)

- **13/13 rows resolved** to explicit arm owners; **0 rows kept AMBIG**. All 13 RECs accepted after live-code verification against the current tree (post batch-1.5 dead-code purge, HEAD `5df276c1b`). Every marker grep-verified: exactly 1 hit per bot file (rando + chosenone mirror; CDSE/CE-shared strings noted where the file is common/ and has no mirror).
- Code-drift findings (not disagreements): **V37.4**'s DeployEvaluator arm is INERT in the current tree (variable computed, never read; the penalty it modulated is hardcoded 0.0f under V40) — the resolution covers the only live arm. **V29.5-buddy**'s line refs drifted to CSE ~3089-3200. No row's code VANISHED in the purge; all 13 point at live current code.
- **V27 scope note**: resolution covers the reservation arms (BE/PE/ME-maintenance) only; the ME buddy-protect arm is a different rule still needing its own home (solo-formation candidate) — flagged above, not silently reassigned.
- §2 table placements and §7 counts are UNCHANGED by this resolution (V24.2 still sits in the battle-weapons table, etc.); physical row re-homing + count updates happen at the next registry regen / migration batch.
- m00235 point 2 (exact stable source markers for ALL 357 rows, replacing base-tag first-hit anchors) remains a **separate batch task**; this section covers only the 13 ex-AMBIG rows.
