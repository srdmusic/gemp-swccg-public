...
**V292 MOVE residual owner note:** the existing `MoveTransitPolicy`, `MoveAbilityPolicy`, `MoveDestinationPolicy`, `MoveObjectiveConsolidationPolicy`, `MoveSpyFollowPolicy`, and `MoveLandoStayPolicy` owners contain the remaining scalar MOVE arithmetic from the three mirrored adapters. The adapters still own all recognition, GEMP/card/objective observations, loops, catches, logs, replacement-score application, action types, insertion, terminal control, and exact CSE order. V292 is rooted at V291 `b75bec70d286279de06216c9198e0a645bfbe386`; its source gates freeze adapter order, battleground catch behavior, and Rando/ChosenOne parity.
