...
When both Chief Chirpa's Hut printings are offered, both bots prefer exact virtual `214_19` because it supplies the free capture route. Exact parent and child provenance prevents generic action text from receiving objective credit. BHBM preserves and funds its exact Emperor download. While capture movement Force is owed, deployment snapshots the engine's exact normal target-specific payment, including fractional base cost rounded up and extra Force. Unknown, changed, special, foreign-source, or targetless costs fail closed.

Rando and Chosen One preserve the exact Force needed to finish the capture route. Proven landspeed, shuttle, embark, disembark, docking-bay transit, Vader's Castle, Rise My Friend, Vader's Machination, and source-legal transport choices are evaluated through their real parents and children. Direct Vader deployment to an open vehicle at the target's site and a legal open-carrier move with Vader aboard receive the same capture band and live Force reserve as direct Vader movement. Enclosed carriers, unrelated passengers, hostile formations, and non-present targets fail closed through exact present-place semantics. Transport remains approach-only because the final cost is established after destiny. Unknown routes remain neutral.

Formation Safety, the capture Force reserve, sole-enabler retention, and the I Feel The Conflict predictor remain active when the opponent currently shows zero power. The last capture enabler, payoff card, Vader group, target group, and the physical carrier containing either group survive avoidable Force loss, battle forfeit, or movement. A duplicate Vader outside the carrier releases unnecessary carrier protection. Safe I Feel The Conflict battle credit remains bounded.

Both bots preserve the captive-or-present-with-Vader back-side chain, including nested carriers, while pursuing source-legal transfer, duel, crossover, and BHBM Throne Room payoff. An opponent-owned Throne Room counts where card source permits it. Native engine tests execute TIGIH capture and flip, BHBM capture and flip, transfer legality, source-modified targets, captive stability, the end-turn Force payment, and strict crossover boundaries.

Production edits are limited to shared AI facts and policy plus mirrored Rando and Chosen One adapters. Tests remain under the AI test tree. No engine, game, card Java, objective profile, deck-library, database, or client source changed. The exact 61-path candidate passed test compilation, the focused Capture gate `214/0/0/0` across 27 suites, and the separate Endor compatibility gate `48/0/0/0`, for `262/0/0/0` across 28 suites. Mirrored winner tests include the open-carrier capture route, open-vehicle deploy, hostile same-site disembark rejection, and exact carrier Force reserve. Diff, engine/card scope, and clean async package gates passed. All 151 top-level and nested classes produced by the 29 changed AI sources byte-match `target/classes`, the server jar, and `web.jar`. Packaged server jar SHA-256 is `29ab732c91354d22d009bc7ad154b93e5769c24862bb121c717c6cd92165b014`; packaged `web.jar` SHA-256 is `e8f2f86c77fa3bd04565c1bee966adac33b3fe626a3e4fbf4f3929bde52da0b4`. Capture behavior is present on the current aggregate JVM, but that mutable aggregate is not used as exact-candidate deployment proof. A zero-table exact-candidate reload and fresh live-game branch firing remain separate gates.

Revert the single Capture objective family Batch Seven commit. First Order Reigns Batch Six and all earlier objective-audit commits remain independent.
## 2026-07-26: TDIGWATT objective family Batch Eight

Classic `109_12` flips only with Dark Deal on table, Dark occupation of Bespin system, and Dark occupation of Bespin: Cloud City. Its back flips only when Dark Deal is canceled, the opponent controls Bespin system, or Bespin is blown away. Virtual `226_12` flips when Dark controls at least three Bespin locations and Light controls fewer than three. Its back stays stable at a tie and flips only when Light controls strictly more Bespin locations.

Both bots recognize the exact physical objective upload action and its real Reserve Deck child prompt. Classic accepts Bespin system, Bespin: Cloud City, Dark Deal, or Cloud City Occupation. Virtual accepts Dark Deal, Vader's Bounty, or only Special Edition Bespin `223_8`; Cloud City Bespin `5_164` remains rejected. Exact parent and child provenance prevents wrong-print cross-talk.

Exact source-stable classic Dark Deal deployment receives `TDIGWATT.109_12.DEPLOY.ENGINE.DARK_DEAL +900`, and exact source-stable classic Cloud City Occupation receives `TDIGWATT.109_12.DEPLOY.ENGINE.CLOUD_CITY_OCCUPATION +900`. A classic Dark Deal projection receives `+600` for advancing the law or `+1200` when it completes both occupation legs. A Dark Deal or Cloud City Occupation that would immediately cancel is categorically rejected. The unchanged classic source performs the actual flip.

With two Bespin locations controlled, an exact non-spy, non-operative character projection scores the empty third Bespin site with `TDIGWATT.226_12.DEPLOY.COMPLETE_FRONT +1200`; unknown or contested projections remain neutral. Both public bots choose that destination, spend the native deploy cost, and fire the unchanged virtual flip. The prompt includes legal Dark Deal (V) `223_9` on Special Edition Bespin `223_8` as a competing action. It remains a legal engine card, receives no false virtual flip-leg bonus, and does not steal the third-control deployment. The exact source-granted Lando landspeed route receives `+600` on the parent and exact safe destination. It remains a paid regular move using the engine-computed cost. An unrelated optional Force drain is vetoed only when its exact cost would consume the reserved movement Force. A real Battle Order cost-three drain is rejected while the paid one-Force Lando landspeed route completes the third Bespin control and fires the same native flip. Other regular-move mechanisms remain outside this typed route.

Back-side objective credit is added only after existing battle and formation safety approve the battle. Classic receives `+80` for an alien and Imperial pair or `+160` when one of your Ugnaughts participates. Virtual receives `+80` for its pair, plus `+20` for Lando or `+40` when Lobot permits two adjustments. On the source-backed native `BATTLE_DESTINY` path, the virtual destiny action beats Pass, adds one to Dark's live draw, subtracts one from Light's live draw, and rejects the harmful direction. Other destiny types remain neutral because owner alone does not prove which direction helps.

No false off-table board projection was added for Force loss. Existing V21 behavior protects classic Dark Deal as a required flip card and preserves the narrower virtual pullable-card treatment. `FORCE_LOSS.RETAIN` and `FORFEIT.RETAIN` have source-law policy tests, but no dedicated TDIGWATT runtime adapter is claimed.

Boundary math is explicit. Pull parent `+200` beats Pass `5-8`; child `+400` selects the legal temp card. Lando `+600` exceeds the strongest current safe ordinary Control contribution of `+440` by `160`. Classic engine deploy `+900` beats Pass only after exact source-persistence proof. Safe typed effects bypass the legacy `-800` gate, while known immediate cancellation is a hard veto. Battle payoff remains capped at `200` and cannot bypass a safety veto.

Production edits are limited to shared AI facts and policy plus mirrored Rando and Chosen One adapters. Tests remain under the AI test tree. No engine, game, card Java, decision metadata, database, deck library, client, or runtime objective profile changed.

The sequential 15-suite TDIGWATT family and shared-battle gate passed `74/0/0/0`, including both public bots choosing the third-control deploy over legal Dark Deal (V), rejecting a legal Cloud City Occupation that would immediately cancel, selecting a source-stable Cloud City Occupation with the exact `+900`, safe battle-destiny direction, and the shared battle caller. Exact TDIGWATT commit `7f2b29067` passed an isolated full reactor with `2723/0/0/26`; its packaged server jar SHA-256 is `4069ede08143b630bd68d07d91032e3abdb102c15b3d7ed3b1e4d1eabd4da2d4` and its packaged `web.jar` SHA-256 is `453000db00392ec17637613f81ddb069a8e8f27f8ec0fe3a9ef39a79c0de2ae8`. All four mirrored bot adapter pairs remain normalized-identical.

Compatibility aggregate `codex/tdigwatt-shield-live` at `93f0fd2c0` passed an isolated full reactor with `2735/0/0/26`. After a zero-table freeze, its packaged server jar SHA-256 was `fa0f0e56ec9700872d40fdd3023112b8213ed24f09385a18af5ae672c66aab3a` and its deployed `web.jar` SHA-256 was `8c1633ae5a4e32d8320ebdf055ee4bfb42c243516786b2440d6087d2e46fe6de`. The host and running-container hashes matched. The fresh JVM started at `2026-07-26T21:53:58.172539586Z` with restart count `0`, reached operational mode, returned HTTP 200, restored all four intended server switches, exposed both TDIGWATT bot paths in loaded bytecode, and left the hall empty with no real startup error or exception. The unfinished Endor shield-slot experiment was absent from the deployed artifact.

Deterministic unchanged-engine scenarios reach both actual flip triggers and the virtual battle-destiny adjustment. Source, decision, sequence, parity, controlled flip, package, and fresh-JVM deployment gates are proven. No live replay or post-deployment TDIGWATT branch firing is claimed.
