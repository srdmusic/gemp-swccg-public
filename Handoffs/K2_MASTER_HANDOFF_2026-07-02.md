...
- **A. Bug fixes / gameplay** → `Handoffs/K2_HANDOFF_2026-07-02_fable5-onboarding.md`. STATUS UPDATE 2026-07-06: TDIGWATT Bug A is DONE (V177 category rescue, `fcec408b9`, live-verified). Still queued: Bug B (V29 BESPIN-FIRST demanding an Executor the objective forbids) and the V61c battle-intent refinement (§ "ALSO QUEUED" in that file). Five NEW confirmed bugs from the 07-04 overlap audit await Steve's queue decision — see the execution handoff §7.
- **B. Reorganization / consolidation** → **`Handoffs/K2_REORG_EXECUTION_HANDOFF_2026-07-06.md`** (SUPERSEDES the 06-30 consolidation file as the workstream entry; written for Opus-model sessions while Fable is unavailable). It operationalizes `resources/RANDO_REORG_PLAN_2026-07-02.md` (the design authority: phase-based section architecture + T0-T4 ladder, absorbing the old 8-move plan) and carries the verified overlap audit (`resources/Rando_Overlap_Audit_2026-07-04.xlsx`, 30 confirmed findings). The 06-30 file stays as detail reference for the original 8 moves + council traps.
