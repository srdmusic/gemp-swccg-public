...
A sophisticated, project-aware verification agent. Claude invokes it via
...
and reads project state without Claude's biases.
...
1. **PR #3260 base mismatch** — Claude pushed an "AI-only" PR but based
...
2. **Card images missing after rebuild** — Claude ran `unzip -oq`
   ...
   Claude declared "fixed" without running `ls` on the destination.

Both failures share the same root cause: Claude trusted the action
...
Claude reads the matching reference file, copies the prompt template,
...
- **PASS** — all checks succeeded. Claude can report success to Steve.
- **WARN** — minor issues that should be flagged but don't block.
  Claude tells Steve about them.
- **FAIL** — at least one check failed. Claude MUST fix the
...
HTTP codes, log line counts) so Claude can confirm independently.
...
This skill is expected to get smarter over time. Claude (the main
session, not the agent) maintains it. After these events, Claude
...
2. **A new type of risky operation** — create a 5th (or 6th, etc.)
   ...
   `/Users/steve/.claude/projects/-Users-steve-gemp-swccg-public/memory/feedback_verify_before_done.md`.
...
SKILL.md or `references/` is Claude's responsibility, not the
