...
public class Card501_042 extends AbstractNormalEffect {
    ...
    protected List<Modifier> getGameTextWhileActiveInPlayModifiers(SwccgGame game, PhysicalCard self) {
        ...
        modifiers.add(new ImmuneToAttritionLessThanModifier(self, Filters.Devastator, 5));
        return modifiers;
    }
    ...
    protected List<TopLevelGameTextAction> getGameTextTopLevelActions(final String playerId, final SwccgGame game, final PhysicalCard self, int gameTextSourceCardId) {
        ...
        if (GameConditions.isOncePerTurn(game, self, playerId, gameTextSourceCardId, gameTextActionId)
            ...
            action.setActionMsg("Deploy Mustafar or a private platform from Reserve Deck");
            // Update usage limit(s)
            action.appendUsage(
                    new OncePerTurnEffect(action));
            // Perform result(s)
            action.appendEffect(
                    new DeployCardFromReserveDeckEffect(action, Filters.or(Filters.Mustafar_system, Filters.titleContains("Private Platform")), true));
            actions.add(action);
        }
        ...
        if (GameConditions.isOncePerTurn(game, self, playerId, gameTextSourceCardId, gameTextActionId)
            ...
            final TopLevelGameTextAction action = new TopLevelGameTextAction(self, playerId, gameTextSourceCardId, gameTextActionId);
            action.setText("Take Devastator into hand from Reserve Deck");
