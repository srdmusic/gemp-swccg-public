 ...
 */
public class Card501_086 extends AbstractObjective {
    public Card501_086() {
        ...
        setGameText("Deploy Jedi Council Chamber (with Prophecy Of The Force there), Ewok Village, and I Feel The Conflict. " +
                "For remainder of game, you may not deploy [Episode I] or [Episode VII] Padawans or Jedi (except Yoda). Luke may not be captured by Bring Him Before Me unless there are two cards stacked on Insignificant Rebellion during any move phase. " +
    ...
    protected RequiredGameTextTriggerAction getGameTextAfterDeploymentCompletedAction(String playerId, SwccgGame game, final PhysicalCard self, final int gameTextSourceCardId) {
        ...
        action.appendEffect(
                new AddUntilEndOfGameModifierEffect(action,
                        new MayNotDeployModifier(self, Filters.and(Filters.or(Icon.EPISODE_I, Icon.EPISODE_VII), Filters.or(Filters.Jedi, Filters.padawan), Filters.except(Filters.Yoda)), playerId), null));
        action.appendEffect(
                new AddUntilEndOfGameModifierEffect(action,
