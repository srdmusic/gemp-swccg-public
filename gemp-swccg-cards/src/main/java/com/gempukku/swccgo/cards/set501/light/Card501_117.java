 ...
 */
public class Card501_117 extends AbstractNormalEffect {
    public Card501_117() {
        ...
        setGameText("If Watch Your Step on table, deploy on table. Corran Horn is a smuggler. Once per character, when you deploy Corran, Mara, Mirax or Talon Karrde to a Tatooine location, may take any one card from Used Pile into hand; reshuffle. Once per game, may deploy Mos Eisley from Reserve Deck; reshuffle. Opponent may not cancel or modify Force drains at battlegrounds where you have two smugglers. [Immune to Alter.]");
    ...
    protected List<TopLevelGameTextAction> getGameTextTopLevelActions(final String playerId, SwccgGame game, final PhysicalCard self, int gameTextSourceCardId) {
        GameTextActionId gameTextActionId = GameTextActionId.I_MUST_BE_ALLOWED_TO_SPEAK_SMUGGLERS_BLUES__DOWNLOAD_MOS_EISLEY;
        ...
        if (GameConditions.isOncePerGame(game, self, gameTextActionId)
            ...
            action.setText("Deploy Mos Eisley from Reserve Deck");
            // Update usage limit(s)
            action.appendUsage(
                    new OncePerGameEffect(action));
            // Perform result(s)
            action.appendEffect(
                    new DeployCardFromReserveDeckEffect(action, Filters.Mos_Eisley, true));
            return Collections.singletonList(action);
        }
        return null;
    }
