...
public class Card501_160 extends AbstractNormalEffect {
    public Card501_160() {
        ...
        setGameText("If Credits Will Do Fine, deploy on Mos Espa on table. Opponent's first Force lost from Force drains here is stacked on Credits Will Do Fine. If you just deployed Jar Jar, Padme, or a Jedi here, may take Either Way, You Win into hand from Reserve Deck; reshuffle.");
    ...
    protected List<OptionalGameTextTriggerAction> getGameTextOptionalAfterTriggers(String playerId, final SwccgGame game, EffectResult effectResult, final PhysicalCard self, int gameTextSourceCardId) {
        ...
        // Check condition(s)
        if (TriggerConditions.justDeployedTo(game, effectResult, playerId, Filters.or(Filters.Jar_Jar, Filters.Padme, Filters.Jedi), Filters.Mos_Espa)
