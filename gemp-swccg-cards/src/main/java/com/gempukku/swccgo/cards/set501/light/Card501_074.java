 ...
 * Title: Aim High (V)
 */
public class Card501_074 extends AbstractDefensiveShield {
    public Card501_074() {
    ...
    protected List<Modifier> getGameTextWhileActiveInPlayModifiers(final SwccgGame game, PhysicalCard self) {
        ...
        Condition hasDeployedABattlegroundSystem = new Condition() {
            @Override
            public boolean isFulfilled(GameState gameState, ModifiersQuerying modifiersQuerying) {
                return GameConditions.hasDeployedAtLeastXCardsThisGame(game, opponent, 1, Filters.battleground_system);
            }
        };
        List<Modifier> modifiers = new LinkedList<>();
        modifiers.add(new ModifyGameTextModifier(self, Filters.Court_Of_The_Vile_Gangster, new UnlessCondition(hasDeployedABattlegroundSystem), ModifyGameTextType.COURT_OF_THE_VILE_GANGSTER__MAY_NOT_DEPLOY_STARSHIPS));
        return modifiers;
    }
