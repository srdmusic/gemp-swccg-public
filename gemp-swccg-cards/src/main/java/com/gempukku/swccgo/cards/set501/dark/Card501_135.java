...
public class Card501_135 extends AbstractLostInterrupt {
    public Card501_135() {
        ...
        setGameText("Once per game, if on top of your Lost Pile, may retrieve into hand. Add one battle destiny; for remainder of turn, opponent may not modify or reset either player’s total battle destiny or ability. (If a smuggler in that battle, opponent’s destiny draws are each -1).");
                    ...
                    new RespondablePlayCardEffect(action) {
                        ...
                        protected void performActionResults(Action targetingAction) {
                            ...
                            action.appendEffect(new AddUntilEndOfBattleModifierEffect(action,
                                    new EachDestinyModifier(self, new InBattleCondition(self, Filters.smuggler), new ConstantEvaluator(-1), game.getOpponent(self.getOwner())), "Make opponent's destiny draws -1 if a smuggler in battle"));

                            //modify total battle destiny
                            action.appendEffect(new AddUntilEndOfTurnModifierEffect(action,
                                    new MayNotModifyTotalBattleDestinyModifier(self, null, opponent),null));

                            //modify total ability
//                            action.appendEffect(new AddUntilEndOfTurnModifierEffect(action,
//                                    new MayNotHaveTotalAbilityReducedModifier(),null));

                            //reset total battle destiny
                            action.appendEffect(new AddUntilEndOfTurnModifierEffect(action,
                                    new MayNotResetTotalBattleDestinyModifier(self, null, opponent),null));
