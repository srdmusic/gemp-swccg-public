...
public class Card501_108 extends AbstractJediMaster {
    public Card501_108() {
        ...
        setLore("");
        setGameText("While 'communing': You may not deploy [Permanent Weapon] or [Maintenance] cards; your immunity to attrition is limited to < 3; once per turn, may deploy a battleground with two [Dark Side] from Reserve Deck; reshuffle; once per game, may retrieve 1 Force.");
    ...
    public List<Modifier> getWhileStackedModifiers(SwccgGame game, PhysicalCard self) {
        ...
        modifiers.add(new MayNotDeployModifier(self, Filters.or(Icon.PERMANENT_WEAPON, Icon.MAINTENANCE), new CommuningCondition(self), self.getOwner()));
        modifiers.add(new ImmunityToAttritionLimitedToModifier(self, Filters.your(self), new CommuningCondition(self),3));
        return modifiers;
    }
