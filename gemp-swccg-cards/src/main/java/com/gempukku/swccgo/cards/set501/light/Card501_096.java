 ...
 */
public class Card501_096 extends AbstractRebel {
    public Card501_096() {
        super(Side.LIGHT, 2, 5, 4, 6, 6, "Kanan Jarrus, Jedi Knight", Uniqueness.UNIQUE);
        setLore("");
        setGameText("If you just deployed Chopper, Ezra, Hera, Kanan, Sabine, or Zeb, to same or related location, may draw top card of Used Pile. Once during your draw phase, if Kanan did not move this turn, may peek at cards in your Used Pile. Immune to attrition < 5.");
        addPersona(Persona.KANAN);
        addIcons(Icon.WARRIOR, Icon.VIRTUAL_SET_17);
        setTestingText("Kanan Jarrus, Jedi Knight (ERRATA)");
    }
    ...
    protected List<OptionalGameTextTriggerAction> getGameTextOptionalAfterTriggers(final String playerId, SwccgGame game, EffectResult effectResult, final PhysicalCard self, int gameTextSourceCardId) {
        ...
        if (TriggerConditions.justDeployedToLocation(game, effectResult, playerId, filter, Filters.sameOrRelatedLocation(self))
                && GameConditions.hasUsedPile(game, playerId)) {
