 ...
 * Type: Interrupt
 * Subtype: Used
 * Title: Local Defense & Put That Down
 */
public class Card501_090 extends AbstractUsedInterrupt {
    public Card501_090() {
    ...
    protected List<PlayInterruptAction> getGameTextTopLevelActions(final String playerId, SwccgGame game, PhysicalCard self) {
        GameTextActionId gameTextActionId = GameTextActionId.LOCAL_DEFENSE_PUT_THAT_DOWN__UPLOAD_INDEPENDENT_STARSHIP;
        ...
        if (GameConditions.canTakeCardsIntoHandFromReserveDeck(game, playerId, self, gameTextActionId)) {
            ...
            // Allow response(s)
            action.allowResponses("Take an [Independent] starship into hand from Reserve Deck",
                    new RespondablePlayCardEffect(action) {
                        @Override
                        protected void performActionResults(Action targetingAction) {
                            // Perform result(s)
                            action.appendEffect(
                                    new TakeCardIntoHandFromReserveDeckEffect(action, playerId, Filters.and(Icon.INDEPENDENT, Filters.starship), true));
                        }
                    }
            );
    ...
    protected List<PlayInterruptAction> getGameTextOptionalAfterActions(final String playerId, SwccgGame game, EffectResult effectResult, final PhysicalCard self) {
        // Check condition(s)
        Filter yourCharacterOrStarship = Filters.and(Filters.your(self), Filters.or(Filters.non_Jedi_character, Filters.starship), Filters.canBeTargetedBy(self, TargetingReason.TO_BE_HIT));
