 ...
 * Set: Set 22
 * Type: Effect
 * Title: Vader's Ambition
 */
public class Card501_018 extends AbstractNormalEffect {
    public Card501_018() {
        super(Side.DARK, 4, PlayCardZoneOption.YOUR_SIDE_OF_TABLE, "Vader's Ambition", Uniqueness.UNIQUE, ExpansionSet.PLAYTESTING, Rarity.V);
        setLore("Join me, and together we can rule the galaxy as father and son.");
        setGameText("Deploy on table if Vader’s Malediction on table. Once per turn, may [download] a Mustafar location or a lightsaber. " +
                "At the start of opponent's control phase, Vader may relocate (as a regular move) between Chasm Walkway and opponent’s battleground site.  " +
                "[Immune to Alter.]");
        addIcons(Icon.CLOUD_CITY, Icon.VIRTUAL_SET_22);
        addImmuneToCardTitle(Title.Alter);
        setTestingText("Vader's Ambition");
    }
