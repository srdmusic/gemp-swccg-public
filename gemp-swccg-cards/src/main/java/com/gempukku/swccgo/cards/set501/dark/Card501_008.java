...
public class Card501_008 extends AbstractNormalEffect {
    public Card501_008() {
        ...
        setGameText("Deploy on table. Once per turn, may deploy Baniss or Cantina from Reserve Deck; reshuffle. During your draw phase, may peek at the top X cards of Used Pile; take one into hand, where X = number of your aliens at Cantina. Immune to Blue Milk. [Immune to Alter.]");
    ...
    protected List<TopLevelGameTextAction> getGameTextTopLevelActions(final String playerId, SwccgGame game, final PhysicalCard self, int gameTextSourceCardId) {
        ...
        if (GameConditions.isOnceDuringYourPhase(game, self, playerId, gameTextSourceCardId, gameTextActionId, Phase.DRAW)
            ...
            if (alienCount > 0) {
                ...
                final TopLevelGameTextAction action = new TopLevelGameTextAction(self, gameTextSourceCardId, gameTextActionId);
                action.setText("Peek at top "+alienCount+" card"+ GameUtils.s(alienCount)+" of Used Pile");
                action.setActionMsg("Peek at top "+alienCount+" card"+ GameUtils.s(alienCount)+" of Used Pile and take one into hand");
                ...
                // Perform result(s)
                action.appendEffect(
                        new PeekAtTopCardsOfUsedPileAndChooseCardsToTakeIntoHandEffect(action, playerId, alienCount, 1, 1));
                actions.add(action);
            }
        }
