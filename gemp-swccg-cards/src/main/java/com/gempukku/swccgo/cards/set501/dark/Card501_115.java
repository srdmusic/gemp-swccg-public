...
public class Card501_115 extends AbstractNormalEffect {
    public Card501_115() {
        ...
        setGameText("Deploy on table. Once per game, may [upload] Finalizer. Once per character, when you deploy Hux, Kylo, Phasma, or Snoke to an [Episode VII] battleground (or any battleground if Tuanul Village not on table), may take any one card from Used Pile into hand; reshuffle. [Immune to Alter.]");
        addIcons(Icon.EPISODE_VII, Icon.VIRTUAL_SET_4);
        addImmuneToCardTitle(Title.Alter);
        setTestingText("Bow To The First Order (ERRATA)");
    }
    ...
    protected List<OptionalGameTextTriggerAction> getGameTextOptionalAfterTriggers(String playerId, final SwccgGame game, EffectResult effectResult, final PhysicalCard self, int gameTextSourceCardId) {
        ...
        Filter battlegroundFilter = GameConditions.canSpot(game, self, Filters.Tuanul_Village) ? Filters.and(Icon.EPISODE_VII, Filters.battleground) : Filters.battleground;
        // Check condition(s)
        if (TriggerConditions.justDeployedTo(game, effectResult, playerId, Filters.and(Filters.character, Filters.or(Filters.Hux, Filters.Kylo, Filters.Phasma, Filters.Snoke)), battlegroundFilter)
