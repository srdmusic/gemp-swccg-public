 ...
 */
public class Card501_126 extends AbstractUsedOrLostInterrupt {
    public Card501_126() {
        ...
        setGameText("USED: If The Hyperdrive Generator’s Gone on table, take [Tatooine] Anakin, [Tatooine] Obi-Wan, or Skywalker Hut into hand from Reserve Deck; reshuffle. LOST: If opponent just stacked a card on Credits Will Do Fine, for remainder of turn your Force drains may not be reduced.");
    ...
    protected List<PlayInterruptAction> getGameTextTopLevelActions(final String playerId, final SwccgGame game, final PhysicalCard self) {
        ...
        if (GameConditions.canSpot(game, self, Filters.The_Hyperdrive_Generators_Gone)
            ...
            // Allow response(s)
            action.allowResponses("Take [Tatooine] Anakin, [Tatooine] Obi-Wan, or Skywalker Hut into hand from Reserve Deck",
                    new RespondablePlayCardEffect(action) {
                        @Override
                        protected void performActionResults(Action targetingAction) {
                            // Perform result(s)
                            action.appendEffect(
                                    new TakeCardIntoHandFromReserveDeckEffect(action, playerId, Filters.or(Filters.and(Icon.TATOOINE, Filters.or(Filters.Anakin, Filters.ObiWan)), Filters.title("Tatooine: Skywalker Hut")), true));
                        }
                    }
            );
            actions.add(action);
