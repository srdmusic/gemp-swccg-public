        ...
        for (int i = 0; i < actionIds.size(); i++) {
            ...
            if (card != null) {
                ...
                if (blueprint != null) {
                    ...
                    // ───── OLD INLINE OBJECTIVE BLOCKS (V83/V110/V108/V86/V88/V99) — consolidated into
                    // ObjectiveAnalyzer.getDeployObjectiveAdjustments() 2026-07-07; kept commented per the
                    // comment-out-superseded-logic rule (strip in a clean build). DO NOT re-enable alongside
                    // the call above or scores would double. ─────
...
//                    // will get destroyed."
//                    //
//                    // My Lord, Is That Legal? / I Will Make It Legal sets weapon
//                    // destiny -6 at Galactic Senate, protecting low-power senators
//                    // from weapon fire. ANYWHERE ELSE, normal weapon destiny applies
//                    // and senators (typically ability 2-3, low power) get killed.
//                    // The objective also REQUIRES 2-3 senators at Galactic Senate
//                    // to stay flipped — deploying senators elsewhere strands the
//                    // win condition.
//                    //
//                    // Type-by-API per Steve's standing rule: Filters.senator uses
//                    // Keyword.SENATOR; Filters.Galactic_Senate uses Title match.
//                    {
//                        com.gempukku.swccgo.ai.models.rando.strategy.ObjectiveAnalyzer mlObj =
//                            context.getObjectiveAnalyzer();
...
//                                    }
//                                }
//                                // V83.1 (Steve, 2026-05-19): only penalize when target is identifiable.
//                                // For generic "Deploy" actions (no location in text), the actual
//                                // location is picked in a separate CardSelection step — penalizing
//                                // here would block the deploy before location-selection runs.
//                                if (mlTargetLoc != null) {
...
//                    }
//
//                    // === V110 (Steve, 2026-05-20): MY LORD — HOLD NON-SENATOR DEPLOYS UNTIL A NON-SENATE SITE EXISTS ===
//                    // Real incident replay lcvfliepk63snbew: Admiral Ozzel kept deploying to
//                    // Senate because Senate was the only top-location available at deploy
//                    // time. V99 correctly penalized Senate as destination, but the deploy
//                    // ACTION still ran — and the CardSelection step had only Senate as
//                    // candidate, so engine forced him there. Fix: penalize the deploy ACTION
//                    // itself when MLITL active AND card is non-senator AND no non-Senate
//                    // ground site is available yet. Holds the non-senator in hand until a
//                    // docking bay / non-Senate site is deployed.
//                    if (card != null && blueprint != null
//                            && blueprint.getCardCategory() == CardCategory.CHARACTER
//                            && gameState != null && game != null) {
//                        com.gempukku.swccgo.ai.models.rando.strategy.ObjectiveAnalyzer v110Obj =
//                            context.getObjectiveAnalyzer();
...
//                                    v110IsSenator = true;
//                                } else {
//                                    String v110Lore = blueprint.getLore();
//                                    if (v110Lore != null && v110Lore.toLowerCase(Locale.ROOT)
//                                            .contains("senator")) {
//                                        v110IsSenator = true;
//                                    }
//                                }
//                                if (!v110IsSenator) {
//                                    // Check if any non-Senate SITE (battleground or not) is on
//                                    // table where we have or could have presence.
//                                    boolean v110HasNonSenateSite = false;
//                                    try {
//                                        for (PhysicalCard loc : gameState.getTopLocations()) {
//                                            if (loc == null || loc.getBlueprint() == null) continue;
//                                            if (com.gempukku.swccgo.filters.Filters.Galactic_Senate
//                                                    .accepts(gameState, game.getModifiersQuerying(), loc)) continue;
//                                            // SITE subtype only (characters need a site, not a system)
//                                            if (loc.getBlueprint().getCardSubtype()
//                                                    == com.gempukku.swccgo.common.CardSubtype.SITE) {
//                                                v110HasNonSenateSite = true;
//                                                break;
//                                            }
//                                        }
//                                    } catch (Exception ignore) { /* */ }
//                                    if (!v110HasNonSenateSite) {
//                                        action.addReasoning(
...
//                    }
//
//                    // === V108 (Steve, 2026-05-20): MY LORD — PRIORITIZE DEPLOYING SENATORS FROM HAND ===
//                    // Replay h3iqvadi8rxzha23: Edcel Bar Gane (senator, in hand all game)
//                    // never deployed because Rando preferred Maarek/Ozzel/Tyranus/Sidious.
//                    // V88 boosts senator→Senate (destination) but doesn't boost the deploy
//                    // action itself in the queue. V108 adds +500 to deploy actions where
//                    // (a) MLITL objective active and (b) card is a senator by lore OR keyword.
//                    // Senator detection via lore + keyword (per feedback_senator_in_lore_not_keyword).
//                    if (card != null && blueprint != null
//                            && blueprint.getCardCategory() == CardCategory.CHARACTER
//                            && gameState != null && game != null) {
...
//                                    v108IsSenator = true;
//                                } else {
//                                    String v108Lore = blueprint.getLore();
//                                    if (v108Lore != null && v108Lore.toLowerCase(Locale.ROOT)
//                                            .contains("senator")) {
//                                        v108IsSenator = true;
//                                    }
//                                }
//                                if (v108IsSenator) {
//                                    action.addReasoning(
//                                        "V108 MY LORD: senator '" + card.getTitle()
//                                            + "' in hand — prioritize deploy (flip target)",
//                                        500.0f);
//                                    LOG.warn("V108 MY LORD: BOOST deploy senator {} → +500",
//                                        card.getTitle());
//                                }
//                            }
//                        }
...
//                    // Neimoidian in lore and is a pilot they should deploy only
//                    // to a capital ship if capital ship is on the table. If
//                    // capital ship is not on table they can deploy anywhere else."
//                    //
//                    // Blockade Flagship (the deck's signature capital ship) gives
//                    // permanent pilot ability +2 and once-per-game pulls a
//                    // Neimoidian pilot aboard. Neimoidian pilots deployed to
//                    // ground sites are wasted — they belong on the ship piloting it.
//                    //
//                    // Type-by-API per Steve's standing rule: Species.NEIMOIDIAN +
//                    // Icon.PILOT detected via Filters, capital ship via CardSubtype.
//                    {
//                        com.gempukku.swccgo.ai.models.rando.strategy.ObjectiveAnalyzer invObj =
//                            context.getObjectiveAnalyzer();
...
//                                // Find any friendly capital ship on the table.
...
//                                    // V86.1 (Steve, 2026-05-19): only act when action text is
//                                    // SPECIFIC enough to identify the target. Generic "Deploy"
//                                    // gets a target picked in CardSelection later — don't pre-block.
...
//                                    }
//                                }
//                                // No capital ship on table → no preference, let other scoring decide.
//                            }
//                        }
//                    }
//
...
//                    // Squabbling Delegates — but NEVER deployed them. Senators
//                    // are low-power and get blocked by solo-deploy/buddy-system
//                    // penalties (V47 etc.). At Galactic Senate the objective
//                    // provides weapon destiny -6, so senators are SAFE there.
//                    //
//                    // This rule gives a strong positive bonus to senators going
//                    // to Galactic Senate so they OVERRIDE the solo-deploy
//                    // penalties that keep them in hand. Required to satisfy the
//                    // flip condition (3 senators at Senate).
//                    {
//                        com.gempukku.swccgo.ai.models.rando.strategy.ObjectiveAnalyzer mlObj88 =
//                            context.getObjectiveAnalyzer();
...
//                                    && com.gempukku.swccgo.filters.Filters.senator.accepts(
//                                        gameState, game.getModifiersQuerying(), card)) {
//                                // Action text contains Galactic Senate?
...
//                    // Inverse of V88: senators belong AT Galactic Senate, non-senators do NOT
//                    // (unless defending). Pre-V99, Rando deployed Maarek Stele + Admiral Ozzel
//                    // to the Senate on turn 1 with zero opponent presence — wasting deploys at
//                    // a site where non-senators get NO weapon-destiny-6 protection.
//                    //
...
//                                && blueprint.getCardCategory() == CardCategory.CHARACTER
//                                && gameState != null && game != null) {
//                            // V99-LORE (Steve, 2026-05-20): senator detection via LORE + Keyword.
//                            // Only ~29 of 35 senator cards have addKeyword(Keyword.SENATOR);
//                            // the rest mark "senator" only in lore text.
//                            boolean v99IsSenator = false;
//                            if (blueprint.hasKeyword(com.gempukku.swccgo.common.Keyword.SENATOR)) {
//                                v99IsSenator = true;
//                            } else {
//                                String v99Lore = blueprint.getLore();
//                                if (v99Lore != null && v99Lore.toLowerCase(Locale.ROOT)
//                                        .contains("senator")) {
//                                    v99IsSenator = true;
//                                }
//                            }
...
//                                            gameState, game.getModifiersQuerying(), loc)) {
//                                        v99Senate = loc;
//                                        break;
//                                    }
//                                }
//                                if (v99Senate != null) {
//                                    String v99ActionLower = actionText.toLowerCase(Locale.ROOT);
//                                    boolean targetsSenate = v99ActionLower.contains(
//                                        v99Senate.getTitle().toLowerCase(Locale.ROOT));
//                                    if (targetsSenate) {
//                                        // Sum friendly SENATOR power at the Senate
...
//                                            try {
//                                                pcLoc = game.getModifiersQuerying()
...
//                                                card.getTitle(), v99OpponentPower, v99FriendlySenatorPower),
//                                                -1500.0f);
                    ...
                    if (false /* V67aj SUPERSEDED V136 */ && category == CardCategory.CHARACTER && card != null && gameState != null
                        ...
                        try {
                            ...
                            for (PhysicalCard ajLoc : gameState.getTopLocations()) {
                                ...
                                com.gempukku.swccgo.ai.models.rando.strategy.ObjectiveAnalyzer ajObj =
                                    context.getObjectiveAnalyzer();
...
//                             try {
//                                 v115Criteria = com.gempukku.swccgo.ai.models.rando.evaluators
//                                     .CardSelectionEvaluator.v70ExtractDeployCriteria(card.getBlueprint().getGameText());
...
//                                     try {
//                                         matchesCriteria = com.gempukku.swccgo.ai.models.rando.evaluators
