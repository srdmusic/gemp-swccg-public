...
public class ChatServer extends AbstractServer {
    ...
    private final int _userIdleSeconds;

    public ChatServer() {
        _userIdleSeconds = parseInt(ApplicationConfiguration.getProperty("chat.userIdle.seconds"), 600);
    }
    ...
    public ChatRoomMediator createChatRoom(String name, boolean muteJoinPartMessages, int secondsTimeoutPeriod, Set<String> allowedUsers, boolean allowSpectatorsToChat, boolean playtesting) {
        ChatRoomMediator chatRoom = new ChatRoomMediator(name, muteJoinPartMessages, secondsTimeoutPeriod, _userIdleSeconds, false, allowedUsers, allowSpectatorsToChat, playtesting);
        try {
    ...
    public ChatRoomMediator createPrivateChatRoom(String name, boolean muteJoinPartMessages, Set<String> allowedUsers, int secondsTimeoutPeriod) {
        ChatRoomMediator chatRoom = new ChatRoomMediator(name, muteJoinPartMessages, secondsTimeoutPeriod, _userIdleSeconds, true, allowedUsers, false, false);
        try {
