...
public class DeployPhasePlanner {
    ...
    // Accepted-response lifecycle. Assessment itself is pure; only accepted engine
    // dispositions enter this cursor.
    private DeployTransaction currentTransaction;
    private DeployTransaction lastTerminalTransaction;
    private String lastInvalidationReason;
    private SwccgGame lifecycleGame;
    private long gameEpoch = 1;
    private final Set<BlockedAttempt> blockedAttempts = new HashSet<>();

    private record BlockedAttempt(long gameEpoch,
                                  int gameIdentity,
                                  int turn,
                                  com.gempukku.swccgo.common.Phase phase,
                                  String playerId,
                                  String attemptId) {
    }
     ...
     */
    public void reset() {
        terminateCurrentTransaction("planner reset");
        currentPlan = null;
        lastPlanTurn = -1;
        currentGame = null;
    ...
    public DeployTransaction getCurrentTransaction() {
        return currentTransaction;
    }

    public DeployTransaction getLastTerminalTransaction() {
        return lastTerminalTransaction;
    }

    public String getLastInvalidationReason() {
        return lastInvalidationReason;
    }

    /**
     * Pure plan assessment. The legacy planner uses instance fields internally, so the
     * assessment runs on a short-lived twin and leaves this owner's lifecycle untouched.
     */
    public DeploymentPlan assessPlan(SwccgGame game, String playerId, Side side) {
        DeployPhasePlanner assessmentPlanner =
                new DeployPhasePlanner(deployThreshold, battleForceReserve);
        assessmentPlanner.setObjectiveAnalyzer(objectiveAnalyzer);
        return assessmentPlanner.createPlan(game, playerId, side);
    }

    /** Current accepted plan when valid for this turn, otherwise a pure fresh assessment. */
    public DeploymentPlan planForDecision(SwccgGame game, String playerId, Side side) {
        if (game == null) {
            return currentPlan != null ? currentPlan.assessmentCopy() : null;
        }
        GameState gameState = game != null ? game.getGameState() : null;
        int turn = gameState != null ? gameState.getPlayersLatestTurnNumber(playerId) : -1;
        if (currentPlan != null && lastPlanTurn == turn) {
            return currentPlan.assessmentCopy();
        }
        return assessPlan(game, playerId, side);
    }

    public ForceObligationVector assessForceObligations(SwccgGame game,
                                                         GameState gameState,
                                                         String playerId) {
        return ForceObligationVector.from(
                ForceReserveService.compute(game, gameState, playerId),
                battleForceReserve);
    }

    /** Reuse the accepted parent's vector for every child stage. */
    public ForceObligationVector forceObligationsForDecision(
            DeployFacts facts, SwccgGame game) {
        Objects.requireNonNull(facts, "facts");
        if (matchesCurrentTransaction(facts)) {
            return currentTransaction.forceObligations();
        }
        GameState gameState = game != null ? game.getGameState() : null;
        return assessForceObligations(game, gameState, facts.playerId());
    }

    /** Owns the retained forced-destination PULL guard; evaluators consume this result. */
    public ForcedDestinationDeploySafety.Assessment assessForcedDestinationDeploySafety(
            SwccgGame game,
            GameState gameState,
            String playerId,
            PhysicalCard source,
        ...
        if (source == null || destination == null || source.getBlueprint() == null) {
            return ForcedDestinationDeploySafety.assess(
                    false, objectiveState, null, false);
        }
        if (objectiveState
                == ForcedDestinationDeploySafety.ObjectiveState.UNFLIPPED_TARGET_NAMED) {
            return ForcedDestinationDeploySafety.assess(
                    true, objectiveState, null, false);
        }
        try {
            SwccgCardBlueprint blueprint = source.getBlueprint();
            Float buddyCost = null;
            List<PhysicalCard> hand = gameState != null
                    ? gameState.getHand(playerId) : null;
            if (hand != null) {
                for (PhysicalCard card : hand) {
                    if (card == null || card.getBlueprint() == null
                            || card.getPermanentCardId() == source.getPermanentCardId()
                            || card.getBlueprint().getCardCategory()
                                != CardCategory.CHARACTER) {
                        continue;
                    }
                    Float cost = card.getBlueprint().getDeployCost();
                    if (cost != null && (buddyCost == null || cost < buddyCost)) {
                        buddyCost = cost;
                    }
                }
            }
            ForceObligationVector obligations = assessForceObligations(
                    game, gameState, playerId);
            float spendableForce = gameState != null
                    ? Math.max(0, gameState.getForcePileSize(playerId)
                        - obligations.plannerReserve())
                    : 0;
            Float sourceCost = blueprint.getDeployCost();
            FormationSafety.CharacterDeployCheck formation =
                    FormationSafety.assessCharacterDeploy(
                            game, gameState, playerId, source,
                            blueprint.hasPowerAttribute() ? blueprint.getPower() : null,
                            blueprint.hasAbilityAttribute() ? blueprint.getAbility() : null,
                            false, destination, spendableForce, sourceCost,
                            buddyCost, null);
            boolean weakSoloNoPlan = formation.state()
                    == FormationSafety.CharacterDeployState.ALLOWED
                    && FormationSafety.weakSoloNoPlan(
                        game, gameState, playerId,
                        blueprint.hasAbilityAttribute() ? blueprint.getAbility() : null,
                        false, destination, buddyCost);
            return ForcedDestinationDeploySafety.assess(
                    true, objectiveState, formation, weakSoloNoPlan);
    ...
    public DeploymentPlan createPlan(SwccgGame game, String playerId, Side side) {
        ...
        ForceObligationVector forceObligations =
                assessForceObligations(game, gameState, playerId);
        if (forceObligations.maintenanceObligation() > 0) {
            LOG.warn("V22.3 MAINTENANCE: Reserving {} total Force for maintenance upkeep",
                    forceObligations.maintenanceObligation());
        }
        int effectiveForce = forceAvailable - forceObligations.plannerReserve();
    ...
    /**
     * Produces the one immutable selected-route assessment. No owner state changes here.
     * The exact frozen legacy wire is already known, so parent assessment binds the
     * physical candidate that would actually be submitted.
     */
    public DeployAssessment assessDecision(DeployFacts facts,
                                           String exactLegacyWire,
                                           DeploymentPlan assessedPlan,
                                           SwccgGame game) {
        return assessDecision(
                facts, exactLegacyWire, assessedPlan,
                forceObligationsForDecision(facts, game), game);
    }
    ...
    public DeployAssessment assessDecision(DeployFacts facts,
        ...
        if (facts.route() == DeployRoute.DEPLOY_PARENT) {
            DeployFacts.ParentCandidate selected =
                    selectedParentCandidate(facts, exactLegacyWire);
            if (selected == null) {
                DeployFacts.ParentCandidate first = facts.parentCandidates().get(0);
                formation = DeployFormationAssessment.unknown(
                        first.sourceCard(), first.orderedDestinations(),
                        "the accepted wire is not a DEPLOY candidate");
            } else if (isBlockedAttempt(facts, selected, game)) {
                formation = new DeployFormationAssessment(
                        DeployFormationAssessment.Verdict.ALL_DESTINATIONS_BLOCKED,
                        selected.sourceCard(), selected.orderedDestinations(),
                        null, null,
                        "the exact accepted attempt was already cancelled at its child boundary");
            } else {
                formation = assessParentFormation(
                        selected, assessedPlan, game, obligations, facts.playerId());
            }
        } else if (matchesCurrentTransaction(facts)) {
            formation = currentTransaction.formation();
            obligations = currentTransaction.forceObligations();
            if (isDeferredPullFormation(formation)) {
                formation = assessDeferredPullFormation(
                        facts, assessedPlan, game, obligations);
            }
        } else {
            formation = DeployFormationAssessment.unknown(
                    facts.sourceCard(), childDestinations(facts),
                    "no accepted parent transaction is available for this child");
        }

        if (facts.route() == DeployRoute.DEPLOY_V170_UNDERCOVER
                && facts.opponentActiveDrain().isUnknown()) {
            return null;
        }
        return DeployAssessment.compatibility(facts, formation, obligations);
    }

    /** Enter the lifecycle only after the mediator reports engine acceptance. */
    public void acceptDecision(DeployFacts facts,
        ...
        Objects.requireNonNull(acceptedWire, "acceptedWire");
        if (facts.route() == DeployRoute.DEPLOY_PARENT) {
            DeployFacts.ParentCandidate selected =
                    selectedParentCandidate(facts, acceptedWire);
            if (selected == null || isBlockedAttempt(facts, selected, game)) {
                return;
            }
            observeLifecycleGame(game);
            DeployTransaction.Key key = new DeployTransaction.Key(
                    gameEpoch,
                    System.identityHashCode(game),
                    facts.turn(),
                    facts.phase(),
                    facts.playerId(),
                    selected.attemptId(),
                    Integer.parseInt(facts.decisionId()));
            Float deployCost = selected.deployCost().isKnown()
                    ? selected.deployCost().value() : null;
            DeployTransaction accepted = DeployTransaction.snapshot(
                    key,
                    selected.sourceCard(),
                    selected.sourceZone(),
                    selected.ordinal(),
                    selected.actionWireId(),
                    selected.orderedDestinations(),
                    selected.orderedBuddyCandidates(),
                    assessment.formation(),
                    assessment.forceObligations(),
                    deployCost).parentAccepted(acceptedWire);
            if (selected.selectedBuddy() != null) {
                accepted = accepted.buddyObserved(
                        selected.orderedBuddyCandidates(), selected.selectedBuddy());
            }
            currentTransaction = accepted;
            currentPlan = assessedPlan != null ? assessedPlan.assessmentCopy() : null;
            lastPlanTurn = facts.turn();
        ...
        currentTransaction = currentTransaction.formationAccepted(
                assessment.formation());
        if (facts.selectedBuddy() != null) {
            if (currentTransaction.selectedBuddy() == null) {
                currentTransaction = currentTransaction.buddyObserved(
                        facts.orderedBuddyCards(), facts.selectedBuddy());
            } else if (!currentTransaction.selectedBuddy().equals(facts.selectedBuddy())) {
                terminateCurrentTransaction("selected buddy drifted from the accepted cursor");
                return;
            }
        }
        if (facts.route() == DeployRoute.DEPLOY_DESTINATION) {
            if (acceptedWire.isBlank()) {
                blockCurrentAttempt();
                terminateCurrentTransaction("accepted destination cancellation");
                return;
            }
            DeployDestinationRef.Card destination =
                    selectedChildDestination(facts, acceptedWire);
            if (destination == null) {
                terminateCurrentTransaction(
                        "accepted destination does not match exact child candidates");
                return;
            }
            List<DeployDestinationRef> observed = childDestinations(facts);
            if (currentTransaction.orderedDestinations().isEmpty()) {
                currentTransaction = currentTransaction.destinationsObserved(observed);
            } else if (!currentTransaction.orderedDestinations().equals(observed)) {
                terminateCurrentTransaction(
                        "destination candidates drifted from the accepted parent snapshot");
                return;
            }
            currentTransaction = currentTransaction.destinationAccepted(
                    destination, acceptedWire);
        } else if (facts.route() == DeployRoute.DEPLOY_BUDDY) {
            DeployPhysicalCardRef buddy = selectedBuddy(facts, acceptedWire);
            if (buddy == null) {
                terminateCurrentTransaction(
                        "accepted buddy does not match exact child candidates");
                return;
            }
            if (currentTransaction.orderedBuddyCandidates().isEmpty()) {
                currentTransaction = currentTransaction.buddyObserved(
                        facts.orderedBuddyCards(), buddy);
            } else if (!currentTransaction.orderedBuddyCandidates()
                    .equals(facts.orderedBuddyCards())) {
                terminateCurrentTransaction(
                        "buddy candidates drifted from the accepted cursor");
                return;
            }
            currentTransaction = currentTransaction.buddyAccepted(buddy, acceptedWire);
        } else {
            if (currentTransaction.selectedDestination() == null
                    && facts.orderedDestinationCards().size() == 1) {
                DeployDestinationRef.Card forced = new DeployDestinationRef.Card(
                        facts.orderedDestinationCards().get(0));
                currentTransaction = currentTransaction.destinationObserved(
                        List.of(forced), forced);
            }
            if (acceptedWire.isBlank()) {
                blockCurrentAttempt();
                terminateCurrentTransaction("accepted DEPLOY child cancellation");
                return;
            }
            currentTransaction = currentTransaction.childChoiceAccepted(acceptedWire);
        }
    }

    /** PULL keeps search ownership; this starts only the accepted deploy transition. */
    public void acceptPullDeployChild(PullFacts pullFacts,
                                      String acceptedWire,
                                      DeploymentPlan assessedPlan,
                                      SwccgGame game) {
        if (pullFacts == null || acceptedWire == null || acceptedWire.isBlank()
                || pullFacts.route()
                    != com.gempukku.swccgo.ai.models.common.phase.PullRoute.PULL_DEPLOY_CHILD
                || pullFacts.transactionId() == null
                || pullFacts.sourceZone().isUnknown()) {
            return;
        }
        PullPhysicalCardRef selected = null;
        for (int i = 0; i < pullFacts.candidateCards().size(); i++) {
            PullPhysicalCardRef candidate = pullFacts.candidateCards().get(i);
            if (acceptedWire.equals(pullFacts.candidateWireIds().get(i))) {
                if (selected != null) {
                    return;
                }
                selected = candidate;
        ...
        observeLifecycleGame(game);
        DeployPhysicalCardRef source = new DeployPhysicalCardRef(
                selected.permanentCardId(), selected.currentCardId());
        ForceObligationVector obligations = assessForceObligations(
                game, game != null ? game.getGameState() : null,
                pullFacts.playerId());
        DeployFormationAssessment formation = DeployFormationAssessment.unknown(
                source, List.of(),
                "deploy-from-pile destinations are not exposed until targeting");
        DeployTransaction.Key key = new DeployTransaction.Key(
                gameEpoch,
                System.identityHashCode(game),
                pullFacts.turn(),
                pullFacts.phase(),
                pullFacts.playerId(),
                "PULL-" + pullFacts.transactionId(),
                pullFacts.parentIdentity().decisionId());
        currentTransaction = DeployTransaction.snapshot(
                key,
                source,
                pullFacts.sourceZone().value(),
                pullFacts.parentIdentity().acceptedActionOrdinal(),
                "PULL-" + pullFacts.transactionId(),
                List.of(),
                List.of(),
                formation,
                obligations,
                null).parentAccepted(acceptedWire);
        currentPlan = assessedPlan != null ? assessedPlan.assessmentCopy() : null;
        lastPlanTurn = pullFacts.turn();
        lastInvalidationReason = null;
    }

    /** Reconcile an accepted cursor against the next engine-visible state. */
    public void observeGameState(SwccgGame game,
                                 GameState gameState,
                                 String playerId,
                                 com.gempukku.swccgo.common.Phase phase) {
        boolean changedGame = lifecycleGame != null && lifecycleGame != game;
        if (changedGame) {
            terminateCurrentTransaction("game identity changed");
            currentPlan = null;
            lastPlanTurn = -1;
        }
        observeLifecycleGame(game);
        if (currentTransaction == null || gameState == null) {
            return;
        }

        DeployTransaction.Key key = currentTransaction.key();
        if (!key.playerId().equals(playerId)) {
            terminateCurrentTransaction("decision player changed");
            return;
        }
        if (phase != key.phase()) {
            terminateCurrentTransaction("phase changed before deployment completed");
            return;
        }
        int turn = gameState.getPlayersLatestTurnNumber(playerId);
        if (turn != key.turn()) {
            terminateCurrentTransaction("turn changed before deployment completed");
            return;
        }

        PhysicalCard source = gameState.findCardByPermanentId(
                currentTransaction.sourceCard().permanentCardId());
        if (source == null) {
            terminateCurrentTransaction("source physical identity disappeared");
            return;
        }
        Zone zone = source.getZone();
        if (currentTransaction.selectedDestination()
                instanceof DeployDestinationRef.ZoneDestination destination
                && zone == destination.zone()
                && zone != currentTransaction.sourceZone()) {
            completeCurrentDeployment(source);
            return;
        }
        if (zone != null && zone.isInPlay()) {
            if (currentTransaction.selectedDestination()
                    instanceof DeployDestinationRef.Card expected) {
                if (!matchesAcceptedDestination(gameState, source, expected.card())) {
                    terminateCurrentTransaction(
            ...
            if (currentTransaction.selectedDestination() == null) {
                DeployDestinationRef.Card observed = observedDestination(gameState, source);
                if (observed != null) {
                    List<DeployDestinationRef> candidates = currentTransaction
                            .orderedDestinations().isEmpty()
                                    ? List.of(observed)
                                    : currentTransaction.orderedDestinations();
                    currentTransaction = currentTransaction.destinationObserved(candidates, observed);
                } else if (!currentTransaction.orderedDestinations().isEmpty()) {
                    terminateCurrentTransaction(
                            "deployed destination drifted from the accepted candidates");
                    return;
                }
            }
            completeCurrentDeployment(source);
        ...
        if (zone != currentTransaction.sourceZone()) {
            terminateCurrentTransaction("unexpected source zone drift: " + zone);
            return;
        }
        Float cost = currentTransaction.deployCost();
        if (cost != null) {
            int required = (int) Math.ceil(cost)
                    + currentTransaction.forceObligations().plannerReserve();
            if (gameState.getForcePileSize(playerId) < required) {
                terminateCurrentTransaction(
                        "Force changed and no longer covers deploy plus obligations");
            }
        }
    }

    private DeployDestinationRef.Card observedDestination(GameState gameState,
                                                          PhysicalCard source) {
        List<DeployDestinationRef.Card> matches = new ArrayList<>();
        for (DeployDestinationRef candidate : currentTransaction.orderedDestinations()) {
            if (candidate instanceof DeployDestinationRef.Card card
                    && matchesAcceptedDestination(gameState, source, card.card())) {
                matches.add(card);
            }
        }
        if (matches.size() == 1) {
        ...
        if (!matches.isEmpty() || !currentTransaction.orderedDestinations().isEmpty()) {
            return null;
        }
        PhysicalCard direct = directDeployDestination(source);
        return direct == null ? null : new DeployDestinationRef.Card(
                new DeployPhysicalCardRef(
                        direct.getPermanentCardId(), direct.getCardId()));
    }

    private boolean matchesAcceptedDestination(GameState gameState,
                                               PhysicalCard source,
                                               DeployPhysicalCardRef expected) {
        PhysicalCard direct = directDeployDestination(source);
        if (direct != null) {
            return direct.getPermanentCardId() == expected.permanentCardId();
        }

        PhysicalCard expectedCard = gameState.findCardByPermanentId(expected.permanentCardId());
        if (expectedCard == null) {
            return false;
        }
        if (expectedCard.getZone() == Zone.CONVERTED_LOCATIONS
                && source.getCardId() == expected.currentCardId()) {
            return true;
        }

        String sourceRelation = locationRelation(source, false);
        String expectedRelation = locationRelation(expectedCard, true);
        return sourceRelation != null && sourceRelation.equals(expectedRelation);
    }

    private PhysicalCard directDeployDestination(PhysicalCard source) {
        PhysicalCard destination = source.getAttachedTo();
        if (destination == null) {
            destination = source.getAtLocation();
        }
        if (destination == null) {
            destination = source.getRelatedStarshipOrVehicle();
        }
        return destination;
    }

    private String locationRelation(PhysicalCard card, boolean includeTitle) {
        String relation = card.getPartOfSystem();
        if (relation == null || relation.isBlank()) {
            relation = card.getSystemOrbited();
        }
        if (includeTitle && (relation == null || relation.isBlank())) {
            relation = card.getTitle();
        }
        return relation == null || relation.isBlank() ? null : relation;
    }

    private void completeCurrentDeployment(PhysicalCard source) {
        currentTransaction = currentTransaction.committed().completed();
        if (currentPlan != null) {
            currentPlan.recordDeployment(
                    source.getPermanentCardId(), source.getCardId(),
                    source.getBlueprintId(true));
        }
        lastTerminalTransaction = currentTransaction;
        currentTransaction = null;
        lastInvalidationReason = null;
    }

    public void rejectDecision(DeployFacts facts, String reason) {
        if (facts != null && facts.route() != DeployRoute.DEPLOY_PARENT
                && matchesCurrentTransaction(facts)) {
            terminateCurrentTransaction(reason);
        }
    }

    public void failCurrentAttempt(String reason) {
        if (currentTransaction != null) {
            terminateCurrentTransaction(reason);
        }
    }

    private DeployFormationAssessment assessParentFormation(
            DeployFacts.ParentCandidate candidate,
            DeploymentPlan plan,
            SwccgGame game,
            ForceObligationVector obligations,
            String playerId) {
        if (!candidate.destinationLegalityKnown()) {
            return DeployFormationAssessment.unknown(
                    candidate.sourceCard(), candidate.orderedDestinations(),
                    "ordered destination legality is unavailable at the parent boundary");
        }
        if (candidate.orderedDestinations().isEmpty()) {
            return new DeployFormationAssessment(
                    DeployFormationAssessment.Verdict.ALL_DESTINATIONS_BLOCKED,
                    candidate.sourceCard(), candidate.orderedDestinations(),
                    null, null, "engine preflight exposed no legal destination");
        }
        GameState gameState = game != null ? game.getGameState() : null;
        PhysicalCard source = exactCard(gameState, candidate.sourceCard());
        if (source == null || source.getBlueprint() == null) {
            return DeployFormationAssessment.unknown(
                    candidate.sourceCard(), candidate.orderedDestinations(),
                    "source physical card is unavailable");
        }
        if (candidate.selectedBuddy() != null) {
            PhysicalCard buddy = exactCard(gameState, candidate.selectedBuddy());
            if (buddy == null || buddy.getZone() != Zone.HAND) {
                return DeployFormationAssessment.unknown(
                        candidate.sourceCard(), candidate.orderedDestinations(),
                        "the exact simultaneous deploy buddy is unavailable");
            }
            return new DeployFormationAssessment(
                    DeployFormationAssessment.Verdict.SAFE_SEQUENCE,
                    candidate.sourceCard(), candidate.orderedDestinations(),
                    candidate.sourceCard(), candidate.selectedBuddy(),
                    "engine-bound simultaneous deploy source and buddy");
        }
        if (source.getBlueprint().getCardCategory() != CardCategory.CHARACTER) {
            return new DeployFormationAssessment(
                    DeployFormationAssessment.Verdict.SAFE_SOLO,
                    candidate.sourceCard(), candidate.orderedDestinations(),
                    candidate.sourceCard(), null,
                    "non-character deploy preserves legacy destination selection");
        }
        try {
            if (source.getBlueprint().mayDeployAsUndercoverSpy(game, source)) {
                return DeployFormationAssessment.unknown(
                        candidate.sourceCard(), candidate.orderedDestinations(),
                        "undercover status is chosen after the parent response");
            }
        } catch (RuntimeException e) {
            return DeployFormationAssessment.unknown(
                    candidate.sourceCard(), candidate.orderedDestinations(),
                    "undercover capability could not be established");
        }
        if (gameState == null || candidate.deployCost().isUnknown()) {
            return DeployFormationAssessment.unknown(
                    candidate.sourceCard(), candidate.orderedDestinations(),
                    "formation inputs or deploy cost are unknown");
        }

        DeploymentInstruction instruction = plan != null
                ? plan.getInstructionForPhysicalCard(
                    source.getPermanentCardId(), source.getCardId(),
                    source.getBlueprintId(true)) : null;
        Float buddyCost = cheapestExactBuddyCost(plan, gameState, source);
        Float power = source.getBlueprint().hasPowerAttribute()
                ? source.getBlueprint().getPower() : null;
        Float ability = source.getBlueprint().hasAbilityAttribute()
                ? source.getBlueprint().getAbility() : null;
        float spendableForce = Math.max(0,
                gameState.getForcePileSize(playerId) - obligations.plannerReserve());
        ArrayList<DeployDestinationRef> allowedDestinations = new ArrayList<>();
        ArrayList<DeployDestinationRef> weakSoloNoPlanDestinations = new ArrayList<>();
        boolean overpower = false;
        for (DeployDestinationRef destinationRef : candidate.orderedDestinations()) {
            if (!(destinationRef instanceof DeployDestinationRef.Card cardDestination)) {
                continue;
            }
            PhysicalCard destination = exactCard(gameState, cardDestination.card());
            if (destination == null) {
                return DeployFormationAssessment.unknown(
                        candidate.sourceCard(), candidate.orderedDestinations(),
                        "a legal destination physical card is unavailable");
            }
            FormationSafety.CharacterDeployCheck formationCheck =
                    FormationSafety.assessCharacterDeploy(
                    game, gameState, playerId, source, power, ability, false,
                    destination, spendableForce, candidate.deployCost().value(),
                    buddyCost, null);
            if (formationCheck.state()
                    == FormationSafety.CharacterDeployState.UNKNOWN) {
                return DeployFormationAssessment.unknown(
                        candidate.sourceCard(), candidate.orderedDestinations(),
                        formationCheck.reason());
            }
            if (formationCheck.state()
                    == FormationSafety.CharacterDeployState.ALLOWED) {
                allowedDestinations.add(destinationRef);
                if (FormationSafety.weakSoloNoPlan(
                        game, gameState, playerId, ability, false,
                        destination, buddyCost)) {
                    weakSoloNoPlanDestinations.add(destinationRef);
                }
                overpower |= isOverpowerOpportunity(
                        game, gameState, playerId, source, destination);
            }
        }
        if (allowedDestinations.isEmpty()) {
            return new DeployFormationAssessment(
                    ...
                    candidate.sourceCard(), candidate.orderedDestinations(),
                    null, null,
                    "FormationSafety blocks every source-proven card destination");
        }
        if (overpower) {
            return new DeployFormationAssessment(
                    DeployFormationAssessment.Verdict.OVERPOWER_OPPORTUNITY,
                    candidate.sourceCard(), candidate.orderedDestinations(),
                    allowedDestinations, weakSoloNoPlanDestinations,
                    candidate.sourceCard(), null,
                    "legal destination provides Steve's explicit overpower opportunity");
        }
        DeployPhysicalCardRef buddy = exactBuddy(plan, gameState, source, instruction,
                candidate.orderedDestinations(), spendableForce,
                candidate.deployCost().value());
        if (buddy != null) {
            return new DeployFormationAssessment(
                    DeployFormationAssessment.Verdict.SAFE_SEQUENCE,
                    candidate.sourceCard(), candidate.orderedDestinations(),
                    allowedDestinations, List.of(), candidate.sourceCard(), buddy,
                    "exact first card and buddy share the accepted plan destination and budget");
        }
        String reason = instruction != null && instruction.getReason() != null
                ? instruction.getReason() : "source-proven legal solo deployment";
        DeployFormationAssessment.Verdict verdict =
                isTargetedRescue(reason)
                    ? DeployFormationAssessment.Verdict.TARGETED_RESCUE
                    : DeployFormationAssessment.Verdict.SAFE_SOLO;
        return new DeployFormationAssessment(
                verdict, candidate.sourceCard(), candidate.orderedDestinations(),
                allowedDestinations, weakSoloNoPlanDestinations,
                candidate.sourceCard(), null, reason);
    }

    private boolean isDeferredPullFormation(DeployFormationAssessment formation) {
        return currentTransaction.key().attemptId().startsWith("PULL-")
                && formation.verdict() == DeployFormationAssessment.Verdict.UNKNOWN;
    }

    private DeployFormationAssessment assessDeferredPullFormation(
            DeployFacts facts,
            DeploymentPlan assessedPlan,
            SwccgGame game,
            ForceObligationVector obligations) {
        List<DeployDestinationRef> destinations = childDestinations(facts);
        if (destinations.isEmpty()) {
            return currentTransaction.formation();
        }
        GameState gameState = game != null ? game.getGameState() : null;
        PhysicalCard source = exactCard(gameState, currentTransaction.sourceCard());
        if (source == null || source.getBlueprint() == null) {
            return DeployFormationAssessment.unknown(
                    currentTransaction.sourceCard(), destinations,
                    "deferred PULL source physical card is unavailable");
        }
        FactValue<Float> deployCost;
        try {
            Float cost = game != null && game.getModifiersQuerying() != null
                    ? game.getModifiersQuerying().getDeployCost(gameState, source)
                    : source.getBlueprint().getDeployCost();
            deployCost = cost != null
                    ? FactValue.known(cost, "deploy-phase-planner",
                        "deferred PULL deploy cost")
                    : FactValue.unknown("deploy-phase-planner", "deferred PULL deploy cost",
                        "source blueprint exposes no deploy cost");
        } catch (RuntimeException e) {
            deployCost = FactValue.unknown(
                    "deploy-phase-planner", "deferred PULL deploy cost",
                    "deploy cost query failed: " + e.getClass().getSimpleName());
        }
        DeployFacts.ParentCandidate candidate = new DeployFacts.ParentCandidate(
                currentTransaction.parentActionOrdinal(),
                currentTransaction.parentActionWireId(),
                currentTransaction.key().attemptId(),
                currentTransaction.sourceCard(), currentTransaction.sourceZone(),
                true, destinations, facts.orderedBuddyCards(),
                facts.selectedBuddy(), deployCost);
        DeploymentPlan plan = assessedPlan != null ? assessedPlan : currentPlan;
        return assessParentFormation(
                candidate, plan, game, obligations, facts.playerId());
    }

    private DeployPhysicalCardRef exactBuddy(
            DeploymentPlan plan,
            GameState gameState,
            PhysicalCard source,
            DeploymentInstruction sourceInstruction,
            List<DeployDestinationRef> destinations,
            float spendableForce,
            float sourceCost) {
        if (plan == null || sourceInstruction == null
                || sourceInstruction.getTargetLocationId() == null) {
            return null;
        }
        Integer targetId = parseCardId(sourceInstruction.getTargetLocationId());
        if (targetId == null || destinations.stream().noneMatch(destination ->
                destination instanceof DeployDestinationRef.Card card
                    && card.card().currentCardId() == targetId)) {
            return null;
        }
        for (DeploymentInstruction instruction : plan.getInstructions()) {
            if (instruction == sourceInstruction
                    || !Objects.equals(sourceInstruction.getTargetLocationId(),
            ...
            PhysicalCard buddy = gameState.findCardByPermanentId(
                    instruction.getCardPermanentCardId());
            if (buddy == null || buddy.getCardId() != instruction.getCardCurrentCardId()
                    || buddy.getZone() != Zone.HAND || buddy.getBlueprint() == null
                    || buddy.getBlueprint().getCardCategory() != CardCategory.CHARACTER
                    || sourceCost + instruction.getDeployCost() > spendableForce) {
                continue;
            }
            return new DeployPhysicalCardRef(
                    buddy.getPermanentCardId(), buddy.getCardId());
        }
        return null;
    }

    private Float cheapestExactBuddyCost(DeploymentPlan plan,
                                         GameState gameState,
                                         PhysicalCard source) {
        if (plan == null || gameState == null) {
            return null;
        }
        Float cheapest = null;
        for (DeploymentInstruction instruction : plan.getInstructions()) {
            ...
            cheapest = cheapest == null ? cost : Math.min(cheapest, cost);
        }
        return cheapest;
    }

    private boolean isOverpowerOpportunity(SwccgGame game,
                                            GameState gameState,
                                            String playerId,
                                            PhysicalCard source,
                                            PhysicalCard destination) {
        try {
            ...
            float theirPower = game.getModifiersQuerying().getTotalPowerAtLocation(
                    gameState, destination, opponent, false, false)
                    + FormationSafety.weaponBonusAt(gameState, destination, opponent);
            if (theirPower <= 0) {
                return false;
            }
            Float sourcePower = source.getBlueprint().hasPowerAttribute()
                    ? source.getBlueprint().getPower() : null;
            float ourPower = game.getModifiersQuerying().getTotalPowerAtLocation(
                    gameState, destination, playerId, false, false)
                    + (sourcePower != null ? sourcePower : 0)
                    + FormationSafety.weaponBonusOf(gameState, source);
            return ourPower >= FormationSafety.DOMINANCE_MULTIPLE * theirPower;
        } catch (RuntimeException e) {
            return false;
        }
    }

    private static boolean isTargetedRescue(String reason) {
        String lower = reason.toLowerCase(Locale.ROOT);
        return lower.contains("rescue") || lower.contains("stop bleeding")
                || lower.contains("reinforce losing");
    }

    private DeployFacts.ParentCandidate selectedParentCandidate(
            DeployFacts facts, String wire) {
        DeployFacts.ParentCandidate selected = null;
        for (DeployFacts.ParentCandidate candidate : facts.parentCandidates()) {
            if (candidate.actionWireId().equals(wire)) {
                if (selected != null) {
                    return null;
                }
                selected = candidate;
    ...
    private DeployDestinationRef.Card selectedChildDestination(
            DeployFacts facts, String wire) {
        if (wire.contains(",")) {
            return null;
        }
        DeployDestinationRef.Card selected = null;
        for (DeployPhysicalCardRef candidate : facts.orderedDestinationCards()) {
            if (wire.equals(String.valueOf(candidate.currentCardId()))) {
                if (selected != null) {
                    return null;
                }
                selected = new DeployDestinationRef.Card(candidate);
            }
        }
        return selected;
    }

    private DeployPhysicalCardRef selectedBuddy(DeployFacts facts, String wire) {
        if (wire.contains(",")) {
            return null;
        }
        DeployPhysicalCardRef selected = null;
        for (int i = 0; i < facts.orderedBuddyCards().size(); i++) {
            DeployPhysicalCardRef candidate = facts.orderedBuddyCards().get(i);
            if (wire.equals(facts.orderedBuddyWireIds().get(i))) {
                if (selected != null) {
                    return null;
                }
                selected = candidate;
    ...
    private static List<DeployDestinationRef> childDestinations(DeployFacts facts) {
        return facts.orderedDestinationCards().stream()
                .map(card -> (DeployDestinationRef) new DeployDestinationRef.Card(card))
                .toList();
    }

    private boolean matchesCurrentTransaction(DeployFacts facts) {
        return currentTransaction != null
                && facts.attemptId() != null
                && currentTransaction.key().attemptId().equals(facts.attemptId())
                && currentTransaction.key().turn() == facts.turn()
                && currentTransaction.key().playerId().equals(facts.playerId())
                && currentTransaction.key().phase() == facts.phase()
                && currentTransaction.key().diagnosticParentDecisionId()
                    == facts.parentDecisionId()
                && currentTransaction.parentActionOrdinal()
                    == facts.parentActionOrdinal()
                && currentTransaction.sourceCard().equals(facts.sourceCard());
    }

    private static PhysicalCard exactCard(GameState gameState,
                                          DeployPhysicalCardRef ref) {
        if (gameState == null || ref == null) {
            return null;
        }
        PhysicalCard card = gameState.findCardByPermanentId(ref.permanentCardId());
        return card != null && card.getCardId() == ref.currentCardId() ? card : null;
    }

    private static Integer parseCardId(String value) {
        try {
    ...
    private void observeLifecycleGame(SwccgGame game) {
        if (lifecycleGame == null) {
            lifecycleGame = game;
        } else if (lifecycleGame != game) {
            lifecycleGame = game;
            blockedAttempts.clear();
            gameEpoch++;
        }
    ...
    private boolean isBlockedAttempt(DeployFacts facts,
                                     DeployFacts.ParentCandidate candidate,
                                     SwccgGame game) {
        return blockedAttempts.contains(new BlockedAttempt(
                gameEpoch, System.identityHashCode(game), facts.turn(), facts.phase(),
                facts.playerId(), candidate.attemptId()));
    }

    private void blockCurrentAttempt() {
        if (currentTransaction == null) {
            return;
        }
        DeployTransaction.Key key = currentTransaction.key();
        blockedAttempts.add(new BlockedAttempt(
                key.gameEpoch(), key.gameIdentity(), key.turn(), key.phase(),
                key.playerId(), key.attemptId()));
    }

    private void terminateCurrentTransaction(String reason) {
        if (currentTransaction == null) {
            return;
        }
        lastInvalidationReason = reason;
        lastTerminalTransaction = currentTransaction.terminated(reason);
        if (currentTransaction.formation().verdict()
                == DeployFormationAssessment.Verdict.ALL_DESTINATIONS_BLOCKED
                && currentPlan != null) {
            currentPlan.removeInstructionForPhysicalCard(
                    currentTransaction.sourceCard().permanentCardId(),
                    currentTransaction.sourceCard().currentCardId());
        }
        currentTransaction = null;
    }
