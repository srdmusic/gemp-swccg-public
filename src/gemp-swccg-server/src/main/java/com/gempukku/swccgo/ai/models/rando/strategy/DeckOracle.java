...
public class DeckOracle {
     ...
     * "X from Reserve Deck" phrasings, so it misses cards like Fall Of The Legend
     * ("Search your Reserve Deck, take one Weather Vane into hand"). But the oracle
     * already knows every TITLE in the deck — so instead of parsing verbs, scan the
     * source card's game text for any of OUR catalogued titles. Each hit is a real
     * DeckCard we can then judge by its actual category and current zone — the
     * "resolve to the real card, then decide by type/zone" design.
     *
     * Only titles >= 6 chars are scanned (short titles like "Han"/"Force" would
     * false-match common words). The source card itself is excluded so a card that
     * names itself never self-blocks.
     */
    public List<DeckCard> namedDeckCardsInText(String text, String excludeBlueprintId) {
        List<DeckCard> out = new ArrayList<>();
        if (text == null || text.isEmpty()) return out;
        String lc = text.toLowerCase(Locale.ROOT);
        for (Map.Entry<String, List<DeckCard>> e : catalogByTitle.entrySet()) {
            String title = e.getKey();
            if (title == null || title.length() < 6) continue;
            if (lc.contains(title)) {
                for (DeckCard dc : e.getValue()) {
                    if (dc == null) continue;
                    if (excludeBlueprintId != null && excludeBlueprintId.equals(dc.getBlueprintId())) continue;
                    out.add(dc);
                }
            }
        }
    ...
    /**
     * V130 (Steve, 2026-05-25): Count cards anywhere in deck matching a Filter.
     *
     * Checks ALL player-owned zones — reserve deck, hand, used pile, force pile,
     * lost pile, and any cards currently on table. Used by V131 Tier 1 hard-block:
     * if zero cards anywhere in the deck match the pull effect's target filter,
     * the effect can NEVER succeed and we should never fire it.
     *
     * Example: Emperor Palpatine's "may search reserve for Force Lightning" — if
     * Force Lightning was never in this deck (returns 0), don't waste Emperor's
     * activation on a search that will fail and reveal the deck.
     *
     * Caller should obtain `filter` via {@link #resolveCommonNounToFilter(String)}
     * OR a specific Filters.X constant. NEVER use title-substring matching.
     */
    public int countMatchingInDeck(com.gempukku.swccgo.game.SwccgGame game,
                                    String playerId,
                                    com.gempukku.swccgo.filters.Filter filter) {
        if (game == null || filter == null || playerId == null) return 0;
        int count = 0;
        try {
            com.gempukku.swccgo.game.state.GameState gs = game.getGameState();
            if (gs == null) return 0;
            java.util.List<com.gempukku.swccgo.game.PhysicalCard> allOurCards =
                new java.util.ArrayList<>();
            // Reserve deck
            try { allOurCards.addAll(gs.getReserveDeck(playerId)); } catch (Exception ignored) {}
            // Hand
            try { if (gs.getHand(playerId) != null) allOurCards.addAll(gs.getHand(playerId)); } catch (Exception ignored) {}
            // Used pile
            try { if (gs.getUsedPile(playerId) != null) allOurCards.addAll(gs.getUsedPile(playerId)); } catch (Exception ignored) {}
            // Force pile
            try { if (gs.getForcePile(playerId) != null) allOurCards.addAll(gs.getForcePile(playerId)); } catch (Exception ignored) {}
            // Lost pile
            try { if (gs.getLostPile(playerId) != null) allOurCards.addAll(gs.getLostPile(playerId)); } catch (Exception ignored) {}
            // Table — any permanent cards we own
            try {
            ...
            for (com.gempukku.swccgo.game.PhysicalCard pc : allOurCards) {
                if (pc == null) continue;
                try {
                    if (filter.accepts(gs, game.getModifiersQuerying(), pc)) {
                        count++;
    ...
    /**
     * V130 (Steve, 2026-05-25): Count cards in HAND + on TABLE matching a Filter
     * (excludes piles and reserve). Used by V131 Tier 2 soft-downgrade: when the
     * effect could pull more of something we already have enough of (in hand or
     * in play), don't apply the LOCATION-tier priority bonus.
     */
    public int countMatchingInHandOrTable(com.gempukku.swccgo.game.SwccgGame game,
                                           String playerId,
                                           com.gempukku.swccgo.filters.Filter filter) {
        if (game == null || filter == null || playerId == null) return 0;
        int count = 0;
        try {
            ...
            if (gs == null) return 0;
            try {
                if (gs.getHand(playerId) != null) {
                    for (com.gempukku.swccgo.game.PhysicalCard pc : gs.getHand(playerId)) {
                        if (pc == null) continue;
                        try {
                            if (filter.accepts(gs, game.getModifiersQuerying(), pc)) count++;
                        } catch (Exception ignored) {}
                    }
                }
            } catch (Exception ignored) {}
    ...
    public boolean hasTargetInZone(Zone zone, String... keywords) {
        ...
        for (DeckCard dc : allCards) {
            ...
            for (String kw : keywords) {
                ...
                // Codex m00118 refinement: only when the word DIRECTLY modifying the type word is
                // possessive ("X's <type>") — a possessive elsewhere (icon remnants like
                // "jabba's palace lando") must keep the rescue.
                if (!kwStripped.isEmpty() && kwStripped.contains(" ")
                        && !isPossessiveTypeTarget(kwStripped)) {
                    String lastWord = kwStripped.substring(kwStripped.lastIndexOf(' ') + 1);
    ...
    public static List<String> parseSourceCardPullTargets(String gameText) {
        ...
        java.util.regex.Pattern[] patterns = new java.util.regex.Pattern[] {
            ...
            // V95 (Steve, 2026-05-20): [upload] catches "take X into hand
            // from Reserve" syntax. Needed so My Sister Has It's
            // "[upload] Chief Chirpa's Hut or Guest Quarters" parses correctly.
            java.util.regex.Pattern.compile(
                "\\[upload\\]\\s+([^.;]+?)(?=\\.|;|$)",
                java.util.regex.Pattern.CASE_INSENSITIVE),
