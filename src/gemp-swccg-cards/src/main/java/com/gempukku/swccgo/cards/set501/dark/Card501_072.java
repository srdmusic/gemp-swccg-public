...
public class Card501_072 extends AbstractNormalEffect {
    public Card501_072() {
        super(Side.DARK, 6, PlayCardZoneOption.YOUR_SIDE_OF_TABLE, Title.Despair, Uniqueness.UNIQUE, ExpansionSet.PLAYTESTING, Rarity.V);
    ...
    protected List<Modifier> getGameTextWhileActiveInPlayModifiers(SwccgGame game, final PhysicalCard self) {
        ...
        Filter sameSiteAsFrozenCaptive = Filters.sameSiteAs(self, SpotOverride.INCLUDE_CAPTIVE, Filters.frozenCaptive);

        List<Modifier> modifiers = new LinkedList<Modifier>();
        modifiers.add(new ForceDrainModifier(self, Filters.and(Filters.site, sameSiteAsFrozenCaptive), 1, playerId));
        modifiers.add(new MayNotBePlacedOutOfPlayModifier(self, Filters.My_Favorite_Decoration));
        modifiers.add(new TotalPowerModifier(self, Filters.battleLocation, new AndCondition(new DuringBattleCondition(), new AtCondition(self, Filters.Scum_And_Villainy, sameSiteAsFrozenCaptive)), 3, playerId));
        return modifiers;
    }
}