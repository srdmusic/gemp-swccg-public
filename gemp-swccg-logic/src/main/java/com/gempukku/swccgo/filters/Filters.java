    ...
    public static Filter hasAvailablePassengerCapacity(PhysicalCard card) {
        ...
        return new Filter() {
            ...
            public boolean accepts(GameState gameState, ModifiersQuerying modifiersQuerying, PhysicalCard physicalCard) {
                ...
                if(physicalCard.getBlueprint().getCardCategory() == CardCategory.VEHICLE
                        || physicalCard.getBlueprint().getCardCategory() == CardCategory.STARSHIP){
                    if (!physicalCard.getBlueprint().getValidPassengerFilter(physicalCard.getOwner(), gameState.getGame(), physicalCard, !physicalCard.getZone().isInPlay()).accepts(gameState, modifiersQuerying, card))
                        return false;
                }
