 ...
 */
public abstract class ChooseCardsFromMultiplePilesEffect extends AbstractStandardEffect implements TargetingEffect {
     ...
     */
    protected ChooseCardsFromMultiplePilesEffect(Action action, String playerId, List<Zone> zones, String zoneOwner, int minimum, int maximum, int maximumAcceptsCount, boolean matchPartialModelType, boolean topmost, Filterable filters) {
        super(action);
        _playerId = playerId;
        _zones = new LinkedList<>(zones);
    ...
    public boolean isPlayableInFull(SwccgGame game) {
        ...
        for(Zone zone:_zones) {
            if (!game.getGameState().getCardPile(_zoneOwner, zone).isEmpty())
                result = true;
        }
        return result;
    }
    ...
    public String getChoiceText(int numCardsToChoose) {
        ...
        sb.append("Choose card" + GameUtils.s(numCardsToChoose) + " from ");
        for(Zone zone:_zones) {
    ...
    protected FullEffectResult playEffectReturningResult(final SwccgGame game) {
        ...
        if (!success) {
            ...
            if (!isPerformedEvenIfMinimumNotReached()) {
                game.getUserFeedback().sendAwaitingDecision(_playerId,
                        new ArbitraryCardsSelectionDecision("Verify " + pileText + " after unsuccessful attempt to '" + getChoiceText(_maximum) + "'", cardPile, Collections.<PhysicalCard>emptyList(), 0, 0) {
                            @Override
        ...
        if (!isSkipTriggerPlayerLookedAtCardsInPile() && _zoneOwner.equals(_playerId)) {
            for (Zone zone:_zones)
                _action.appendAfterEffect(new TriggeringResultEffect(_action, new LookedAtCardsInOwnCardPileResult(_zoneOwner, zone)));
        }
